...
use airline;
CREATE TABLE FLIGHTS(FLNO INT PRIMARY KEY,FFROM VARCHAR(15) NOT NULL,TTO VARCHAR(15) NOT NULL,DISTANCE INT,DEPARTS TIMESTAMP,ARRIVES TIMESTAMP,PRICE NUMeric(10,2));
DESC FLIGHTS;
CREATE TABLE AIRCRAFT(AID INTEGER PRIMARY KEY,ANAME VARCHAR(10),CRUISINGRANGE INTEGER);
desc aircraft;
CREATE TABLE EMPLOYEES(EID INTEGER PRIMARY KEY,ENAME VARCHAR(15),SALARY int);
DESC EMPLOYEES;
...
WHERE A.Aid IN (SELECT C.aid
 ...
 SELECT C.eid, MAX(A.cruisingrange) as max
...
SELECT A.aid
FROM Aircraft A
WHERE A.cruisingrange >( SELECT MIN(F.distance) as min
FROM Flights F
WHERE F.ffrom = 'Bangalore' AND F.tto = 'Frankfurt' );
